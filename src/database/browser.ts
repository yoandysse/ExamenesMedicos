...
// Base de datos en memoria para entorno del navegador
...
// Try to load existing data from localStorage on startup
try {
  const savedData = localStorage.getItem('examPlatformQuestions');
  if (savedData) {
    const parsed = JSON.parse(savedData);
    if (Array.isArray(parsed) && parsed.length > 0) {
      memoryDatabase = parsed;
      nextId = Math.max(...memoryDatabase.map(q => q.id)) + 1;
    }
  }
} catch (error) {
  console.warn('Could not load saved questions from localStorage:', error);
}

// Save to localStorage whenever data changes
const saveToStorage = () => {
  try {
    localStorage.setItem('examPlatformQuestions', JSON.stringify(memoryDatabase));
  } catch (error) {
    console.warn('Could not save questions to localStorage:', error);
  }
};

// Funciones para gestión de preguntas (versión navegador)
...
// Funciones para resultados de exámenes (versión navegador)
export const examResultsDB = {
  save: (result: any): void => {
    try {
      ...
      console.warn('Could not save exam result:', error);
    }
  },
  ...
  getAll: () => {
    try {
      ...
      console.warn('Could not load exam results:', error);
      return [];
    }
  },
  ...
  getStats: () => {
    try {
      ...
      console.warn('Could not get exam stats:', error);
      return {
        totalExams: 0,
        averageScore: 0,
        bestScore: 0,
